...
public class SysUserController extends SysHbaseBaseController {
    ...
    @ResponseBody
    public AjaxResult authTableForUser(Long userId, String tables) {
        ...
        final List<SysUserHbaseTable> sysUserHbaseTableList = tableNameList.stream().map(fullTableName -> {
            String namespaceName = fullTableName.split(":")[0];
            String tableId = StrEnDeUtils.encrypt(fullTableName);
            ...
            sysUserHbaseTable.setNamespaceName(namespaceName);
            sysUserHbaseTable.setClusterAlias(clusterCodeOfCurrentSession());
