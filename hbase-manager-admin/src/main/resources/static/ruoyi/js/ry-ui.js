...
(function ($) {
    $.extend({
        ...
        table: {
            ...
            selectShowLabels: function (datas) {
                var styles = ['success', 'info', 'warning', 'primary', 'danger']
                var actions = [];
                $.each(datas, function (index, content) {
                    var rand = Math.floor(Math.random() * styles.length)
                    var style = styles[rand];
                    actions.push($.common.sprintf("<span class='%s'>%s </span>", "badge badge-" + style, content))
                });
                return actions.join(' ');
            },
            // 显示表格指定列
